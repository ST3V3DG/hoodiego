...
export function AccordeonItem({
  ...
  return (
    <details
      open={defaultOpen}
      className={cn(
        "group rounded-sm border border-zinc-800 bg-zinc-900/50 transition-all open:border-zinc-700 items-center justify-center px-4 py-2 font-bold duration-2000 ease-in-out shadow-[4px_4px_0px_var(--color-primary)] hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-[6px_6px_0px_var(--color-primary)] active:translate-x-1 active:translate-y-1 active:shadow-none",
        className,
      )}
    >
      <summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-5 py-4 text-sm font-medium text-primary transition-colors hover:text-primary/80">
        {title}
